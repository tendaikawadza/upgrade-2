...
import io.getarrays.securecapita.rowmapper.PurchaseRowMapper;
...
public class PurchaseRequestsJdbc implements PurchaseRequestsRepository<PurchaseRequests> {
    ...
    public List<PurchaseRequests> list() {
        try {
            return jdbc.query(PurchaseQuery.SELECT_PURCHASE_REQUEST_QUERY, new PurchaseRowMapper());
    ...
    @Override
    public PurchaseRequests get(Long userId) {
        try {
            return jdbc.queryForObject(PurchaseQuery.SELECT_PURCHASE_REQUEST_BY_USER_ID_QUERY, of("userId", userId), new PurchaseRowMapper());
        } catch (EmptyResultDataAccessException exception) {
            throw new ApiException("No PURCHASE REQUESTS found by id: " + userId);
        } catch (Exception exception) {
            log.error(exception.getMessage());
...
// Get the file extension from the original file name
    ...
    @Override
    public void uploadSignature(PurchaseRequests purchaseRequests, MultipartFile image) {
